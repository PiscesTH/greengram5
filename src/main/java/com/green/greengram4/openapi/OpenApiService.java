...
public class OpenApiService {
    ...
    public List<ApartmentTransactionDetailVo> getApartmentTransactionList
        ...
        WebClient webClient = WebClient.builder()
                .filters(exchangeFilterFunctions -> exchangeFilterFunctions.add(logRequest()))  //openApi 요청 로그 찍기 위해 설정 ?
                .uriBuilderFactory(factory)
                .baseUrl(openApiProperties.getApartment().getBaseUrl())
                .build();

        String data = webClient.get().uri(uriBuilder -> {  //data에 xml문장 저장됨
                            UriBuilder ub = uriBuilder.path(openApiProperties.getApartment().getDataUrl())
        ...
        JsonNode jsonNode = om.readTree(data);  //json형태의 문자열을 JsonNode 형식으로 구조화
        List<ApartmentTransactionDetailVo> list = om.convertValue(jsonNode
                                .path("body")
                                .path("items")
                                .path("item"),  //경로 설정. body/items/item
